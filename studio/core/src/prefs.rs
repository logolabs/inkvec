//! Preferences: what is remembered between sessions.
//!
//! The model only: every field, its default and its clamps. Where the preferences are kept
//! is the shell's business -- a JSON file in the platform's config directory on the desktop
//! (`studio/src-tauri/src/settings.rs`), the browser's storage on the web. Either way it
